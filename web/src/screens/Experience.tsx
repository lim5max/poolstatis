...
function SessionTimeline({ surfaces, env }: { surfaces: ExperienceSurface[]; env: string }) {
  ...
  return <Panel title="Session timeline" right={<span className="text-xs text-muted-foreground">known session id · no DOM replay</span>}>
    <div className="flex flex-wrap gap-2"><QueryControls surfaces={surfaces} selected={selected} onSurface={changeSurface} /><Input aria-label="Session ID" value={sessionId} onChange={(event) => changeSessionId(event.target.value)} placeholder="session id" className="min-w-52 flex-1" /><Button onClick={load} disabled={!selected || !sessionId.trim() || busy}>{busy && <Loader2 className="size-4 animate-spin" />}Load session</Button></div>
    {error && <div className="mt-3"><ErrorNote>{error}</ErrorNote></div>}
...
function QueryControls({ surfaces, selected, onSurface, right, button }: { surfaces: ExperienceSurface[]; selected: string; onSurface: (value: string) => void; right?: React.ReactNode; button?: React.ReactNode }) {
  return <div className="flex flex-wrap gap-2"><Select value={selected} onValueChange={onSurface} disabled={surfaces.length === 0}><SelectTrigger aria-label="Capture surface" className="h-9 min-w-44"><SelectValue placeholder="Choose surface" /></SelectTrigger><SelectContent>{surfaces.map((surface) => <SelectItem key={surface.key} value={surface.key}>{surface.name} · {surface.status}</SelectItem>)}</SelectContent></Select>{right}{button}</div>;
}
