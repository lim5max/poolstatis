  ...
  return (
    <Panel title="New feature flag">
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <Field label="Purpose" className="md:col-span-2"><Textarea aria-label="Flag purpose" value={purpose} onChange={(event) => setPurpose(event.target.value)} placeholder="Safely test a clearer call to action at checkout." /></Field>
      </div>
      <div className="mt-4 space-y-2">
        ...
        {variants.map((variant, index) => (
          <div key={variant.id} className="grid gap-2 rounded-panel border p-3 md:grid-cols-[1fr_7rem_1fr_auto]">
...
function ExperimentForm({ flags, metrics, onCreated }: { flags: FeatureFlag[]; metrics: Metric[]; onCreated: () => void }) {
  ...
  return <Panel title="New experiment"><div className="grid gap-3 md:grid-cols-2"><Field label="Key"><Input aria-label="Experiment key" value={key} onChange={(event) => setKey(event.target.value)} placeholder="checkout_copy_test" /></Field><Field label="Name"><Input aria-label="Experiment name" value={name} onChange={(event) => setName(event.target.value)} placeholder="Checkout copy test" /></Field><Field label="Flag"><Select value={flagKey} onValueChange={setFlagKey}><SelectTrigger aria-label="Experiment flag"><SelectValue placeholder="Choose active flag" /></SelectTrigger><SelectContent>{eligibleFlags.map((flag) => <SelectItem key={flag.id} value={flag.key}>{flag.key}</SelectItem>)}</SelectContent></Select></Field><Field label="Primary metric"><Select value={metricKey} onValueChange={setMetricKey}><SelectTrigger aria-label="Experiment primary metric"><SelectValue placeholder="Choose active event metric" /></SelectTrigger><SelectContent>{eligibleMetrics.map((metric) => <SelectItem key={metric.id} value={metric.key}>{metric.key}</SelectItem>)}</SelectContent></Select></Field><Field label="Hypothesis" className="md:col-span-2"><Textarea aria-label="Experiment hypothesis" value={hypothesis} onChange={(event) => setHypothesis(event.target.value)} placeholder="Changing the checkout copy will increase completed purchases." /></Field></div><div className="mt-4 flex flex-wrap items-center justify-between gap-3"><span className="text-xs text-muted-foreground">An experiment starts only when its flag allocates exactly 100%.</span><Button onClick={submit} disabled={!valid || busy || eligibleFlags.length === 0 || eligibleMetrics.length === 0}>{busy ? <Loader2 className="size-4 animate-spin" /> : <Target className="size-4" />}Create experiment</Button></div>{error && <div className="mt-3"><ErrorNote>{error}</ErrorNote></div>}</Panel>;
}
