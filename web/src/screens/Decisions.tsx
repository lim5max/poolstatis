...
function DecisionReview({ detail, onChanged }: { detail: DecisionDetail; onChanged: () => void }) {
  ...
  return <div className="space-y-4">
    ...
    <Panel title="Reproducible query"><pre className="overflow-x-auto rounded-md bg-muted p-3 text-xs">{JSON.stringify(evidence.query_specs, null, 2)}</pre></Panel>
...
function DecisionAutomation({ detail, onChanged }: { detail: DecisionDetail; onChanged: () => void }) {
  ...
  return <>
    <Panel title="Correlation hypotheses" right={<Button size="sm" variant="outline" onClick={explain} disabled={Boolean(busy)}>{busy === 'explain' && <Loader2 className="size-4 animate-spin" />}Explain outcome</Button>}>
      {data.explanations.length === 0 ? <p className="text-sm text-muted-foreground">No bounded explanation snapshot yet. Explain reads registered metrics and trusted properties only; every result is correlation, not causality.</p> : <div className="space-y-2">{data.explanations[0]!.candidates.map((candidate) => <div key={`${candidate.kind}:${candidate.key}`} className="rounded-md border p-3 text-xs"><div className="flex flex-wrap items-center justify-between gap-2"><div><Badge variant="outline">hypothesis</Badge> <code className="ml-1">{candidate.kind}.{candidate.key}</code></div><span>{candidate.strength} · score {candidate.score}</span></div><div className="mt-2 text-sm">{candidate.purpose}</div><div className="mt-1 text-muted-foreground">{candidate.why_considered}</div></div>)}</div>}
    </Panel>
