...
export function sanitizeReplayEvents(input: unknown, options: SanitizeOptions): SanitizedReplayPayload {
  ...
  const events = input.map((candidate, index) => {
    ...
    if (!Number.isInteger(type) || (type as number) < 0 || (type as number) > 4
        || !Number.isFinite(timestamp) || (timestamp as number) < 0) {
  ...
  return {
    ...
    hasCheckout: events.some((event) => event.type === 2),
  };
}
...
function sanitizeValue(value: unknown, context: WalkContext): unknown {
  ...
  const executable = tagName !== null && ['script', 'iframe', 'object', 'embed', 'form', 'base', 'meta'].includes(tagName);
...
function sanitizeAttributes(
  ...
  for (const [rawName, rawValue] of Object.entries(attributes)) {
    ...
    else if (name === 'role' && !/^(?:button|link|dialog|main|navigation|banner|contentinfo|region|heading|list|listitem|img|presentation|none|status|alert|tab|tabpanel|textbox|checkbox|radio|menu|menuitem|progressbar|slider|switch|table|row|cell)$/i.test(value)) value = MASK;
    else if (name === 'type' && !/^(?:button|submit|reset|text|checkbox|radio|range|number|date|time|search)$/i.test(value)) value = MASK;
    else if (forceMask
        || name === 'name'
        || name === 'title'
        || name === 'alt'
