...
const runner = await import(coreModule) as {
  runMcpServer(config: { baseUrl: string; token: string; distribution?: 'source' | 'published-0.6.0' }): Promise<void>;
  validateMcpConfig(env: NodeJS.ProcessEnv): { baseUrl: string; token: string };
};

try {
  await runner.runMcpServer({ ...runner.validateMcpConfig(process.env), distribution: 'published-0.6.0' });
} catch (error) {
