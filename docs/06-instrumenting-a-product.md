...
`@poolstatis/mcp@0.3.0` is the release pin for Web Engagement and Visual
Experience. Hosted deployments may enable this exact pin only after its
registry artifact passes fresh install, initialize, tool-list, and
project-scoped semantic read smoke; future versions remain fail-closed.
