...
| Consent withdrawal or expired retention remains readable | Reads reject an elapsed `delete_after` immediately. Playback holds a shared manifest lock through object validation, append-only view audit and a bounded 15-second HTTP response lease; abort/deadline rolls back that audit and closes the response. Withdrawal takes the conflicting lock, so its completion is a read barrier without allowing stalled readers to exhaust the pool. Tombstone/request audit commits first, then claimed retry jobs with backoff remove committed chunks, crash-orphans and temporary files without poison rows starving later retention. Finalization also scrubs session/distinct/host/token/privacy identifiers from the idempotency tombstone. The original deletion actor is retained only in immutable audit across worker completion. |
| Project deletion leaves replay objects or accepts late writes | After non-replay prerequisites, project deletion atomically raises a replay write barrier and a durable no-FK cleanup job. Manifest creation, upload and completion take a shared project lock and reject once the barrier is raised. The job survives the project cascade and can finish replay tombstones, project-row deletion and a final prefix sweep after request/process failure. Append-only deletion-request/completion audit deliberately keeps the immutable project UUID and actor after that cascade; hosted runtime has only `SELECT/INSERT` on it. |
