...
MVP: stdio-сервер и готовящийся version-pinned npm-пакет `@poolstatis/mcp@0.1.0` (токен только в env). Пока registry smoke не пройден, документация и hosted onboarding используют точный local Core checkout, runner status остаётся `publish_pending`. После fresh-install initialize/list-tools smoke hosted deploy может включить точный pin. Поддерживаемые presets: Claude Code, Claude Desktop, Codex, Cursor, Warp, Windsurf, VS Code/Copilot, Cline, Zed, Continue, Replit, OpenCode, Hermes-style launchers и custom MCP host. Streamable HTTP — отдельный этап после hosted-стабилизации.
