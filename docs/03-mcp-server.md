...
MVP: stdio-сервер (публичный version-pinned npm-пакет `@poolstatis/mcp@0.1.0`, токен в env). Поддерживаемые presets в hosted onboarding: Claude Code, Claude Desktop, Codex, Cursor, Warp, Windsurf, VS Code/Copilot, Cline, Zed, Continue, Replit, OpenCode, Hermes-style launchers и custom MCP host. Все они используют один stdio shape: command, args, env. Hosted deploy включает registry preset только после fresh-install initialize/list-tools smoke для точной версии; до этого runner status остаётся `publish_pending`, а UI показывает local Core fallback. Streamable HTTP — отдельный этап после hosted-стабилизации.
