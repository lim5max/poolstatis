...
MVP: stdio-сервер и version-pinned npm-пакет `@poolstatis/mcp@0.3.0` (токен только в env). Это release pin для Web Engagement и Visual Experience; hosted deploy включает его только после fresh-install initialize/list-tools и project-scoped semantic read smoke exact registry artifact. Непроверенные версии остаются `publish_pending`. Поддерживаемые presets: Claude Code, Claude Desktop, Codex, Cursor, Warp, Windsurf, VS Code/Copilot, Cline, Zed, Continue, Replit, OpenCode, Hermes-style launchers и custom MCP host. Streamable HTTP — отдельный этап после hosted-стабилизации.
