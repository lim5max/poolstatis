...
Migration 027 validates these exact `NOLOGIN NOINHERIT` roles before creating
any policy object. A missing grant aborts 027 in its transaction.
...
The job fails closed until the restricted runtime grants are exact, rolling
event partitions exist for the next 12 months, and operational indexes are
ready. Run it during deploys and on a daily schedule so the next UTC-month
boundary is prepared well before traffic reaches it.
...
the require/backfill functions. It cannot read or mutate
